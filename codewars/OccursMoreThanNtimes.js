...
//Alice and Bob are able to encode the motif as a number. Can you help them to remove numbers such that their list contains each number only up to N times, without changing the order?
...
function deleteNth(arr, N) {
    ...
    arr.filter(function(num) {
      // 결과 배열에서 현재 숫자의 등장 횟수를 계산.
      let count = result.filter(function(x) {
        return x === num;
      }).length;
      
      // 현재 숫자의 등장 횟수가 N보다 작거나 같은 경우에만 결과 배열에 추가.
      if (count < N) {
        result.push(num);
      }
    });
